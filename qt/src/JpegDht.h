// MJPG → JPEG fixup: many UVC cameras emit MJPG frames WITHOUT the Huffman
// table (DHT) segments — decoders like ffmpeg silently substitute the standard
// ITU T.81 Annex K.3 tables, but libjpeg (Qt's JPEG plugin) hard-fails instead
// ("Huffman table 0x00 was not defined"). withStandardDht() detects a DHT-less
// frame and splices the standard tables in front of the SOS marker so libjpeg
// can decode it.
//
// The table bytes below were extracted VERBATIM from a libjpeg-encoded baseline
// JPEG and round-trip validated (strip DHT → re-insert this blob → decode is
// pixel-identical), so they cannot suffer from a hand-typed-constant typo.
#pragma once

#include <QByteArray>

// Standard JPEG Huffman tables (ITU T.81 Annex K.3), extracted verbatim from
// a libjpeg-encoded baseline JPEG. Spliced into MJPG frames that omit their
// DHT segments (common UVC behavior) so libjpeg can decode them.
static const unsigned char kStandardDht[] = {
...
// If `data` is a baseline JPEG with no DHT segment before SOS, return a copy
// with the standard tables inserted in front of SOS. Returns an empty array if
// the frame already has DHT (no fix needed) or isn't parseable as JPEG.
static inline QByteArray withStandardDht(const unsigned char *data, int size) {
    if (size < 4 || data[0] != 0xff || data[1] != 0xd8) return {};   // not JPEG
    int i = 2;
    while (i + 3 < size) {
        ...
        if (m == 0xda) {                             // SOS — splice here
            QByteArray out;
            out.reserve(size + int(sizeof(kStandardDht)));
            out.append(reinterpret_cast<const char *>(data), i);
            out.append(reinterpret_cast<const char *>(kStandardDht), int(sizeof(kStandardDht)));
            out.append(reinterpret_cast<const char *>(data) + i, size - i);
            return out;
        }
        if (m == 0x01 || (m >= 0xd0 && m <= 0xd9)) { i += 2; continue; }   // standalone
        const int len = (data[i + 2] << 8) | data[i + 3];
        if (len < 2) return {};
        i += 2 + len;
    }
    return {};
